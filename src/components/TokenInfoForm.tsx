...
export function TokenInfoForm(props: { onChangeToken?: (tokenData: TokenData | null) => void }) {
	...
	React.useEffect(() => {
		if (props.onChangeToken === undefined) return;

		if (tokenType !== ItemType.ERC721 && tokenType !== ItemType.ERC1155)
			return props.onChangeToken(null); // Invalid token type.

		if (!tokenAddress.startsWith("0x") || tokenAddress.length !== 42)
			return props.onChangeToken(null); // Invalid token address.

		if (!tokenId.match(/^\d+$/))
			return props.onChangeToken(null); // Invalid token ID.
	...
	}, [props, tokenType, tokenAddress, tokenId]);
